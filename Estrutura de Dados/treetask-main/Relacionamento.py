...
class Relacionamento:
    ...
    def adicionar_relacionamento(self, usuario_id1, usuario_id2):
        if usuario_id1 in self.usuarios and usuario_id2 in self.usuarios:
            ...
            print(f"Relacionamento entre {usuario_id1} e {usuario_id2} adicionado com Sucesso!")
        else:
            print(f"Um ou ambos usuários não achados: {usuario_id1}, {usuario_id2}")
    ...
    def mostrar_arvore(self):
        for usuario in self.usuarios.values():
            print(f"{usuario.nome} ({usuario.usuario_id}) -> {[amigo.nome for amigo in usuario.relacionamentos]}")
